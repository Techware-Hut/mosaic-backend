 ...
 * Supports optional geospatial radius search via lat/lng/radius query params.
 * Falls back to text/city/state matching when no geo params are provided.
 */
...
const { isValidCoordinates } = require('../../utils/geocode');

// Only 'nearMe' style params remain unsupported — lat/lng/radius are now implemented
const GEO_UNSUPPORTED_KEYS = ['nearMe', 'nearme'];
...
function parsePublicSearchQuery(query = {}) {
  ...
  // Parse geo params — optional, non-breaking
  const rawLat = parseFloat(query.lat);
  const rawLng = parseFloat(query.lng);
  const lat = isValidCoordinates(rawLat, rawLng) ? rawLat : null;
  const lng = isValidCoordinates(rawLat, rawLng) ? rawLng : null;
  const radiusKm = Math.min(
    MAX_RADIUS_KM,
    Math.max(1, parseFloat(query.radius) || DEFAULT_RADIUS_KM)
  );
...
/**
 * Resolves Business IDs within a geo radius using MongoDB $nearSphere.
 * Returns an array of matching Business ObjectIds, or null if no geo params given.
 * Products are geo-filtered via their vendor's Business.location.
 * Services and Foods are directly geo-filtered via their own location field.
 * However, all 3 types share the same allowedBusinessIds mechanism in the
 * search controller, so this single function handles all 3 listing types.
 */
async function resolveBusinessIdsByGeo(lat, lng, radiusKm = DEFAULT_RADIUS_KM) {
  if (lat == null || lng == null || !isValidCoordinates(lat, lng)) return null;

  const maxDistanceMetres = radiusKm * 1000;
...
async function resolveCombinedBusinessFilters({
  ...
  // Geo radius filter — narrows businesses to those within radiusKm of lat/lng
  const geoIds = await resolveBusinessIdsByGeo(lat, lng, radiusKm);
  if (geoIds !== null) sets.push(geoIds);
