...
function loadController(options = {}) {
  ...
  const Service = {
    ...
    countDocuments: async (filter) => {
      captured.serviceCount = filter;
      return options.serviceTotal ?? (options.serviceRows ? options.serviceRows.length : 1);
    },
    findOne: async () => null,
  ...
  const Food = {
    ...
    countDocuments: async (filter) => {
      captured.foodCount = filter;
      return options.foodTotal ?? (options.foodRows ? options.foodRows.length : 1);
    },
  ...
  const Product = {
    ...
    countDocuments: async (filter) => {
      captured.productCount = filter;
      return options.productTotal ?? (options.productRows ? options.productRows.length : 1);
    },
    findById: () => ({ populate: () => ({ populate: async () => null }) }),
  };
...
test('service badge filter supports Silver and Gold without explicit businessId', async () => {
  ...
  assert.deepEqual(captured.serviceCount, captured.serviceFind);
});
...
test('food badge filter normalizes Bronze without explicit businessId', async () => {
  const { controller, captured } = loadController({
    foodRows: [buildListing('food', ids.bronze)],
  });
  const res = mockResponse();

  await controller.getAllFood({ query: { badge: 'BRONZE', price: 'all' } }, res);

  assert.equal(res.body.success, true);
  assert.deepEqual(normalizeIdSet(captured.foodFind.businessId), [ids.bronze]);
  const badgeFilter = captured.businessFinds.find((filter) => filter.badge);
  assert.deepEqual(badgeFilter.badge.$in, ['Bronze']);
  assert.equal(badgeFilter.isApproved, true);
  assert.equal(badgeFilter.isActive, true);
  assert.deepEqual(captured.foodCount, captured.foodFind);
});
...
test('food badge filter returns empty for explicit nonmatching vendor context', async () => {
  ...
  assert.equal(captured.foodCount, null);
});
...
test('product badge filter shares Bronze normalization and business-scope intersection', async () => {
  ...
  const badgeFilter = captured.businessFinds.find((filter) => filter.badge);
  assert.deepEqual(badgeFilter.badge.$in, ['Bronze']);
  assert.deepEqual(captured.productCount, captured.productFind);
});
