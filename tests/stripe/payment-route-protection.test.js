...
async function invokeLegacyPaymentIntentForOrder(orderOverrides = {}) {
  const ORDER_ID = '507f1f77bcf86cd799439011';
  let createCalls = 0;
  const originalLoad = Module._load;
  const order = {
    ...
    if (request === 'stripe') {
      return () => ({
        paymentIntents: {
          create: async () => {
            createCalls += 1;
            return { id: 'pi_should_not_run', client_secret: 'secret' };
          },
  ...
  const res = mockResponse();
  await createPaymentIntent(
    { body: { orderId: ORDER_ID }, user: { id: String(order.userId) } },
    res
  );
  return { res, createCalls };
}

test('legacy payment route cannot replace an existing authoritative PaymentIntent', async () => {
  const { res, createCalls } = await invokeLegacyPaymentIntentForOrder({
...
test('legacy payment route cannot create a new charge for refunded or closed orders', async () => {
  for (const orderState of [
    { paymentStatus: 'refunded', status: 'refunded' },
    { paymentStatus: 'failed', status: 'cancelled' },
  ]) {
    const { res, createCalls } = await invokeLegacyPaymentIntentForOrder(orderState);
    assert.equal(res.statusCode, 409);
    assert.equal(createCalls, 0);
  }
});
