...
| [#31](https://github.com/Techware-Hut/mosaic-backend/issues/31) | Vendor profile/listings/orders | Business, product, order vendor routes + `listingTierLimits` | 16 new vendor listing/order/stock tests (123 total) | End-to-end vendor flows on prod | Product+variant tier limit aligned; service/food limits pre-existing; see [MVP_BACKEND_VENDOR_PROFILE_LISTINGS_ORDERS_STOCK.md](MVP_BACKEND_VENDOR_PROFILE_LISTINGS_ORDERS_STOCK.md) |
...
4. ~~**#31 Vendor MVP APIs**~~ — **Done (branch `sprint/backend-vendor-profile-listings-orders-stock`):** product+variant tier limits, stock guards, vendor order tests + [MVP_BACKEND_VENDOR_PROFILE_LISTINGS_ORDERS_STOCK.md](MVP_BACKEND_VENDOR_PROFILE_LISTINGS_ORDERS_STOCK.md)
