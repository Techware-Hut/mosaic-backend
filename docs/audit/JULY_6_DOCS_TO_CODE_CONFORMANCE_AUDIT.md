...
| 4 | Features cannot be edited | Fixed via BE #202 | `features` accepted on create (`lib/service/serviceContract.js` L121–133) and update (`serviceController.js` L786–794, in `updatableFields`); model `Service.features: [String]`. FE `EditServiceForm` sends `features` in `PUT /api/service/:id` payload. | `POST /api/service`, `PUT /api/service/:id` | BE: `service-payload-contract.test.js`, `service-publication.integration.test.js`; FE: `services.test.ts` | Implemented / Ready for QA | both | Low |
