...
Release-control status: **Implemented / Ready for QA** for automated verification on production `main`. Manual UAT smoke evidence is still **Evidence Needed**.

Production promotion: backend PR #209 and frontend PR #335 merged 2026-07-07 **before** manual UAT evidence. This is controlled production UAT — **not final launch approval**.
...
| --- | --- | --- | --- | --- |
| #201 | Backend | Backend architecture gap audit docs | `185f73f8eb1073e1921603b5010c9300df46443f` | Implemented / Ready for QA |
| #202 | Backend | Persist service features on create | `2dfef8b9ce1799c93da4e7cc613e31a92fd54d20` | Implemented / Ready for QA |
| #203 | Backend | Admin application status filters API | `de38cf3a3486931bf98791ac4431569b85b8c34e` | Implemented / Ready for QA |
| #204 | Backend | Local delivery vendor state contract | `cc5a34cc13a98546f8bfea1d79c8f05b79573049` | Implemented / Ready for QA |
| #205 | Backend | Service/food Connect optionality backend | `dadc5f4e31c4fa4078250bbffb1a65f770097a6a` | Implemented / Ready for QA |
| #206 | Backend | Service offering counts | `65b89d5dbe22c10adc48628018b3ee7b9b5a7bee` | Implemented / Ready for QA |
| #325 | Frontend | Frontend architecture gap audit docs | `cda41a03256fbaabb09cbadab127f381a1eb5500` | Implemented / Ready for QA |
| #326 | Frontend | Admin status filters UI | `eb2cf772d54a4244d3afbc9cd1e6cbab41de97a3` | Implemented / Ready for QA |
| #327 | Frontend | Admin finalize/profile review decisions | `8f000a158cd2ab8ab7674846443fcf937d0dcbfa` | Implemented / Ready for QA |
| #328 | Frontend | Image-limit copy | `9e6bb7cf8d61ddea7d4de521c058d31053274d3b` | Implemented / Ready for QA |
| #329 | Frontend | Local shipping UI | `b1448f474a8af8f3333e16b134476710c47bcaf4` | Implemented / Ready for QA |
| #330 | Frontend | Connect messaging for service/food vendors | `d41c035f8ef1d01de0100ec03a463360aee6be0c` | Implemented / Ready for QA |
| #331 | Frontend | Edit image gallery preservation | `5f145368820fdbd8af13ff53cd15d9e3dfc6e670` | Implemented / Ready for QA |
| #332 | Frontend | Service offering display | `4a6f3793bb849a1a414a14a733b249936df957bb` | Implemented / Ready for QA |
| #333 | Frontend | Plain-text product descriptions | `aaffd6cdcee760c7fd7b98c02bb2e5d61acb2714` | Implemented / Ready for QA |
...
| --- | --- | --- |
| Preserve `GET /api/featured-products` | Backend active source registers `/featured-products`; frontend active source calls `/api/featured-products` from `lib/api/featured-products.ts`. | Implemented / Ready for QA |
| Do not introduce `/api/products/featured` | Active backend scan over `app.js`, `routes`, `controllers`, and `lib` found no matches. Active frontend scan over `app`, `components`, `lib`, `hooks`, and `utils` found no matches. | Implemented / Ready for QA |
| Stripe webhook and middleware order | Backend contract tests passed. No backend runtime code changed in this pass. | Implemented / Ready for QA |
| Secrets | No `.env`, secret, credential, token, Stripe key, AWS key, DSN, or signed URL values were committed or documented. | Implemented / Ready for QA |

## July 6 Vendor Journey Checklist

No manual browser smoke was marked **Passed Smoke** because safe demo credentials and hosted runtime evidence were not available in this pass. Items with automated proof are marked **Implemented / Ready for QA** and still need client/UAT screenshots.

| # | Checklist item | Status | Owner | Branch/PR evidence | Command/test proof | Screenshot/evidence path | Next action | Rollback note |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Image upload limit message | Implemented / Ready for QA | Frontend | #328 | Frontend build/unit/focused eslint passed. | Evidence Needed | Capture upload UI showing tier limit copy does not say 3 when 6 are allowed. | Revert #328 if copy regresses. |
| 2 | Service count/display | Implemented / Ready for QA | Both | Backend #206, Frontend #332 | Backend integration counts service offerings; frontend service offering unit tests passed. | Evidence Needed | Capture vendor dashboard and public/vendor profile showing expected offerings. | Revert #206/#332 if count/display regresses. |
| 3 | Edit image gallery | Implemented / Ready for QA | Frontend | #331 | Frontend product edit image helper tests passed. | Evidence Needed | Capture edit view before/after save with multiple images. | Revert #331 if gallery persistence regresses. |
| 4 | Feature editing | Implemented / Ready for QA | Backend primary | #202 | Backend unit/integration coverage passed for service create/update feature persistence. | Evidence Needed | Capture service feature edit/save/reopen flow. | Revert #202 if feature persistence regresses. |
| 5 | Service vendor payout/bank setup | Implemented / Ready for QA | Both | Backend #205, Frontend #330 | Backend integration and frontend guard tests show service/food skip payout setup; product vendors still require payout. | Evidence Needed | Capture service vendor final-review messaging. | Revert #205/#330 if payout gating regresses. |
| 6 | Product HTML description | Implemented / Ready for QA | Frontend | #333 | `productDescriptionToPlainText` unit tests passed. | Evidence Needed | Capture product page/card showing no raw tags or unsafe HTML injection. | Revert #333 if description rendering regresses. |
| 7 | Local shipping | Implemented / Ready for QA | Both | Backend #204, Frontend #329 | Backend integration covers vendor state contract; frontend local delivery unit tests passed. | Evidence Needed | Capture same-state and different-state cart/checkout shipping options. | Revert #204/#329 if local eligibility regresses. |
| 8 | Cart quantity decrease | Implemented / Ready for QA | Both | Existing July 6 cart work plus frontend #334 | Backend integration covers decrease by `cartItemId`; frontend #334 preserves update flow while fixing lint. | Evidence Needed | Capture increase/decrease subtotal update in cart. | Revert #334 only if UI behavior regresses; cart logic rollback depends on prior cart PRs. |
| 9 | Coupon cart-value logic | Implemented / Ready for QA | Backend primary | Existing July 6 cart/coupon work | Backend unit/integration covers minimum order validation; frontend still defers to backend. | Evidence Needed | Capture below-minimum rejection and above-minimum backend-approved discount. | Revert coupon backend PRs if validation regresses. |
| 10 | Cart vs checkout amount | Implemented / Ready for QA | Both | Existing July 6 cart/checkout work plus frontend #334 | Backend tests cover order initiation guards; frontend build/unit pass after lint fix. | Evidence Needed | Capture cart total, checkout total, and safe backend total summary. | Revert #334 only if display/request payload regresses. |
...
| 12 | PDF upload | Implemented / Ready for QA | Both | Existing upload fixes | Backend PDF/JPEG upload tests and frontend PDF MIME tests passed. | Evidence Needed | Capture JPEG and PDF upload in hosted/local authenticated UI; do not expose signed URLs. | Revert upload fix only if hosted S3/CORS smoke proves regression. |
| 13 | Admin filters/profile badge review | Implemented / Ready for QA | Both | Backend #203, Frontend #326/#327 | Backend integration covers status filters; frontend admin API unit tests passed. | Evidence Needed | Capture status filters and review detail with private data redacted. | Revert #203/#326/#327 if admin review regresses. |
| 14 | Approval/disapproval/finalize | Implemented / Ready for QA | Both | Frontend #327 plus backend finalize foundation | Backend finalize tests and frontend explicit decision tests passed. | Evidence Needed | Capture reject/change reason, vendor next step, resubmit if supported, approve/finalize. | Revert #327 if finalize UI regresses; backend rollback depends on finalize foundation PR. |
| 15 | Restaurant/service Stripe Connect requirement | Implemented / Ready for QA | Both | Backend #205, Frontend #330 | Frontend guard tests show service and food skip payout before final review; backend optionality tests passed. | Evidence Needed | Capture restaurant/service final review and online-payout messaging. | Revert #205/#330 if Connect optionality regresses. |
...
| Bug | Status | Evidence |
| --- | --- | --- |
| Frontend focused eslint failed on cart/buy-now with 5 React compiler errors | Implemented / Ready for QA | Frontend PR #334; focused July 6 eslint now exits 0 with 0 errors. |
...
Ready for staging UAT: **Implemented / Ready for QA**.
