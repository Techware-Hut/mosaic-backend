...
| **Stripe Connect required only for product online payouts**; service/food/restaurant onboarding, approval, final review, and directory/offline listing publication do **not** require Connect | **Accepted** | Bryan/Rakesh written decision 2026-07-07 (#218); backend enforces via `PAYOUT_REQUIRED_LISTING_TYPES = ['product']` in `utils/businessListingVisibility.js`; `POST /api/orders/initiate` still requires `stripeConnectAccountId` for product checkout | Product + backend | [#218](https://github.com/Techware-Hut/mosaic-backend/issues/218), `tests/marketplace/stripe-connect-payout-policy.test.js`, `tests/integration/business-storefront-publication.integration.test.js` |
