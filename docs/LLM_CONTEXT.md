...
Before starting issue work, read [MVP_BACKEND_PROGRAM_STATUS.md](MVP_BACKEND_PROGRAM_STATUS.md) and inspect the current branch/PR. Exact deploy SHAs, EB version labels, open PR state, and test counts drift quickly and should live in the status hub, CI output, and release proof packs rather than this reusable context file.
...
4. Payload sanitizers after JSON parsing
5. Feature routers via `app.use(prefix, router)`
6. Sentry Express error handler after routes when Sentry is enabled
...
Sentry is wired through `instrument.js`, Express error handling, and `middlewares/sentryHttpCapture.js`; capture depends on Sentry env being enabled in the runtime.
...
| Issue | Title | Notes |
| --- | --- | --- |
| #18 | Sentry monitoring | Wired through `instrument.js` and env-gated runtime capture; verify live capture per runbook |
| #19–#23 | IAM, CORS smoke, staging workflow, rollback doc, push-to-main criteria | Mixed status; verify issue tracker and current runbooks before acting |
...
**Recommended order:** Finish #50 -> #55 (contract docs) -> #56 (test harness) -> #44/#53 (performance) -> security #57/#41 -> observability #58 and any current Sentry verification follow-up.
...
| `npm test` | Run non-integration tests via `scripts/run-unit-tests.js` |
| `npm run test:contract` | Run launch contract checks |
| `npm run test:integration` | Run isolated integration tests with `mongodb-memory-server` |
